...
def logistic_cost_function(X,y, theta):
    ...
    #PDF Seite 17
    J = -1/len(y)*(y @ np.log(h) + (1 - y) @ np.log(1 - h)) 
...
def accuracy_score(y_true,y_pred):
    ...
    #PDF5 Seite 25
    #acc = y_true/y_pred
    #ich starte bei 0 Genauigkeit und vergleiche dann die pred mit den true Werten
    a = 0
    for l in range(len(y_pred)):
        if y_pred[l] == y_true[l]:
            a = a + 1
    acc = a/len(y_pred)        
    return acc
...
def precision_score(y_true,y_pred):
    ...
    #acc Ansatz übernehmen
    #tp = 0
    #fp = 0
    #for l in range(len(y_pred)):
    #   if y_pred[l] == 1 and y_true[l] == 1:
    #       tp = tp + 1
    #  if y_pred[l] == 1 and y_true[l] == 0:
    #       fp = fp + 1
