...
def QuadraticFeatures_fit_transform(X):
    # TODO: berechne Xq
    zeilenlänge = len(X[0,:])
    ##Hinweise Lineare Regression 2, 
    Xq = X
    for n in range(zeilenlänge):
        for i in range(n, zeilenlänge):
            Xq = np.append(X, np.transpose([X[:,n] * X[:,i]]), axis=1)
            print("QudraticFeature-Matrix: \n" + str(Xq)) 
...
def Ridge_fit(X, y, alpha):
    ...
    X_ext = extend_matrix(X)
    ident = np.identity(X_ext.shape[1])
    ident[0][0] = 0
   
    theta = np.linalg.solve(X_ext.T.dot(X_ext) + (alpha * ident), X_ext.T.dot(y))
    return theta
